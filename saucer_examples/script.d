...
@exportd() SEXP generate_numbers(SEXP n_ptr)
{
...
@exportd("dot_product") SEXP dot(SEXP x_sexp, SEXP y_sexp)
{
...
@exportd() SEXP vdmul(SEXP x_sexp, SEXP y_sexp)
{
  size_t n = LENGTH(x_sexp);
  assert(n == LENGTH(y_sexp), "Lengths of the input arrays are not equal");
...
@exportd() SEXP outer_prod_serial(SEXP x, SEXP y)
{
...
@exportd() SEXP outer_prod_parallel(SEXP x, SEXP y)
{
...
@exportd() SEXP outer_prod_types(RVector!(REALSXP) x, RVector!(REALSXP) y)
{
...
@exportd() double[] multiply_arr(double[] x, double[] y)
{
  auto n = x.length;
...
@exportd() double dot_type(double[] x, double[] y)
{
...
*/
@exportd() auto test_strsxp(RVector!(STRSXP) x)
{
...
@exportd() auto test_string(string[] x)
{
  for(long i = 0; i < x.length; ++i)
  {
    x[i] = "Goodbye World";
...
@exportd() auto create_string_vector(size_t n)
{
...
@exportd() auto create_string_matrix(size_t nrow, size_t ncol)
{
...
@exportd() auto create_integer_vector(size_t n)
{
...
@exportd() auto test_attr_1(SEXP item, SEXP name, SEXP value)
{
  setAttrib(item, name, value);
  return item;
}

@exportd() auto test_attr_2(SEXP item, string name, string value)
{
...
@exportd() auto create_d_factor(RVector!(INTSXP) arr)
{
